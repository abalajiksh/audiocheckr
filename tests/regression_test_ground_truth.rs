...
// 
// This test reflects what files ACTUALLY ARE (ground truth), not current detector capabilities.
// Use this to measure detector improvement over time and identify areas needing work.
//
// Expected initial results: ~50-60% detection rate
// Goal: Improve detector to reach 95%+ detection rate
...
fn test_all_audio_files_comprehensive() {
    ...
    println!("\n=== GROUND TRUTH Regression Test Suite ===");
    println!("This test measures actual detector performance against known file defects\n");
    ...
    for (idx, test_case) in test_cases.iter().enumerate() {
        ...
        if result.passed == result.expected {
            ...
            if result.passed && !result.expected {
                ...
                println!("  → Detector missed defects in defective file");
            } else {
                false_positives += 1;
                println!("[{}/{}] ✗ FALSE POSITIVE: {}", idx + 1, test_cases.len(), test_case.description);
                println!("  → Detector flagged clean file as defective");
            }
    ...
    println!("Correctly Identified: {} ({:.1}%)", passed, (passed as f32 / test_cases.len() as f32) * 100.0);
    println!("Incorrect: {} ({:.1}%)", failed, (failed as f32 / test_cases.len() as f32) * 100.0);
    println!("  - False Negatives (missed defects): {}", false_negatives);
    println!("  - False Positives (wrongly flagged): {}", false_positives);

    println!("\n{'='*70}");
    println!("DETECTOR PERFORMANCE ANALYSIS");
    println!("{'='*70}");

    // Calculate metrics for defective files only
    let defective_cases: Vec<_> = test_cases.iter()
        .enumerate()
        .filter(|(_, tc)| !tc.should_pass)
        .collect();

    let detected_defects = defective_cases.iter()
        .filter(|(i, _)| !results[*i].passed)
        .count();

    println!("Defective File Detection:");
    println!("  Total defective files: {}", defective_cases.len());
    println!("  Successfully detected: {} ({:.1}%)", 
        detected_defects,
        (detected_defects as f32 / defective_cases.len() as f32) * 100.0);
    println!("  Missed (false negatives): {}", defective_cases.len() - detected_defects);

    if failed > 0 {
        println!("\n{'='*70}");
        println!("IMPROVEMENT AREAS (First 10 failures)");
        println!("{'='*70}");
        let mut failure_count = 0;
        for (i, result) in results.iter().enumerate() {
            if result.passed != result.expected && failure_count < 10 {
                failure_count += 1;
                let failure_type = if result.passed && !result.expected {
                    "FALSE NEGATIVE (missed)"
                } else {
                    "FALSE POSITIVE (wrong)"
                };

                println!("\n{}: {} [{}]", failure_count, test_cases[i].description, failure_type);
    ...
    // Don't panic - this is a development/benchmark test
    if failed > 0 {
        println!("⚠️  Detector needs improvement in {} areas", failed);
        println!("This is expected - use these results to guide development.");
    } else {
        println!("✅ Perfect detection! Detector is working flawlessly.");
    }
}
...
fn define_test_cases(base: &Path) -> Vec<TestCase> {
    ...
    // These represent what files ACTUALLY are, not what detector currently finds
    // Goal: Improve detector to correctly identify all of these
...
cases.push(TestCase {
    ...
    should_pass: false,
    expected_defects: vec!["BitDepthMismatch".to_string()],
    description: "192kHz upscaled resampled".to_string(),
});
cases.push(TestCase {
...
cases.push(TestCase {
    ...
    should_pass: false,
    expected_defects: vec![],
    description: "96kHz upscaled resampled".to_string(),
});
cases.push(TestCase {
