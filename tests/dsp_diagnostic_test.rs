...
fn diagnostic_dithering_sample() {
    ...
    if !dither_dir.exists() {
        println!(
            "Dithering test directory not found: {}",
            dither_dir.display()
        );
...
fn diagnostic_resampling_sample() {
    ...
    if !resample_dir.exists() {
        println!(
            "Resampling test directory not found: {}",
            resample_dir.display()
        );
        println!("Skipping diagnostic (download test files first)");
        return;
    }
...
fn analyze_output(stdout: &str) {
    ...
    let detectors = [
        (
            "Mp3Transcode",
            "mp3 transcode",
            "❌ FALSE POSITIVE - MP3 detector should not fire on DSP files",
        ),
        (
            "AacTranscode",
            "aac transcode",
            "❌ FALSE POSITIVE - AAC detector should not fire on DSP files",
        ),
        (
            "OggVorbisTranscode",
            "ogg vorbis transcode",
            "❌ FALSE POSITIVE - Vorbis detector should not fire on DSP files",
        ),
        (
            "DitheringDetected",
            "dithering detected",
            "✓ Expected for dithered files",
        ),
        (
            "ResamplingDetected",
            "resampling detected",
            "✓ Expected for resampled files",
        ),
        (
            "BitDepthMismatch",
            "bit depth mismatch",
            "⚠ May be expected for dithered files",
        ),
...
fn test_binary_exists() {
    ...
    assert!(
        binary.exists(),
        "audiocheckr binary not found at {:?}",
        binary
    );
}
