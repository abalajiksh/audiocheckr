...
fn diagnose_control_files() {
    ...
    for r in &results {
        ...
        println!(
            "{} {:<48} {:>6.1} {:>8} {:>8.0}Hz {:>7.1}% {:>8.1}dB/oct  {}",
            status,
        ...
        let mut allure_builder = AllureTestBuilder::new(&test_name)
            .full_name(&format!(
                "diagnostic_test::diagnose::{}",
                sanitize_name(&r.filename)
            ))
            .severity(if r.is_false_positive {
                AllureSeverity::Critical
            } else {
                AllureSeverity::Normal
            })
            .epic("AudioCheckr")
            ...
            .parameter(
                "rolloff_steepness",
                &format!("{:.1} dB/oct", r.rolloff_steepness),
            )
        ...
        allure_builder =
            allure_builder.attach_text("Verbose Analysis Output", &r.stdout, &allure_results_dir);

        if r.is_false_positive {
            false_positive_count += 1;
            let message = format!(
                "FALSE POSITIVE: Control file incorrectly flagged with defects: {:?}",
                r.detected_defects
            );
    ...
    println!("\n--- Rolloff Steepness Analysis ---");
    if !false_positives.is_empty() {
        let fp_steep: Vec<f32> = false_positives
            .iter()
            .map(|r| r.rolloff_steepness)
            .collect();
        ...
        println!(
            "  Min: {:.1} dB/oct  Max: {:.1} dB/oct  Avg: {:.1} dB/oct",
            fp_min, fp_max, fp_avg
        );
    }
    ...
    if !false_positives.is_empty() {
        ...
        println!(
            "   False positives have cutoff ratios as low as {:.1}%",
            fp_min * 100.0
        );
        println!(
            "   Average false positive cutoff ratio: {:.1}%",
            fp_avg * 100.0
        );
        ...
        if fp_min < 0.85 {
            ...
            println!(
                "   SUGGESTION: Lower threshold to {:.0}% or require additional evidence",
                suggested * 100.0
            );
        }
        ...
        println!("\n2. ROLLOFF STEEPNESS:");
        println!(
            "   Average steepness in false positives: {:.1} dB/oct",
            fp_steep_avg
        );
        ...
        println!(
            "   False positives with brick-wall: {} / {}",
            fp_brick_wall_count,
            false_positives.len()
        );
    ...
    println!(
        "Analyzed {} files: {} clean, {} false positives",
        results.len(),
        clean_count,
...
fn parse_verbose_output(filename: &str, output: &str) -> DiagnosticResult {
    ...
    for line in output.lines() {
        ...
        {
            result.has_brick_wall =
                line.contains("Yes") || line.contains("true") || line.contains("detected");
        }
        ...
        if line.contains("MP3") && (line.contains("transcode") || line.contains("Transcode")) {
            if !result
                .detected_defects
                .contains(&"Mp3Transcode".to_string())
            {
        ...
        if line.contains("AAC") && (line.contains("transcode") || line.contains("Transcode")) {
            if !result
                .detected_defects
                .contains(&"AacTranscode".to_string())
            {
        ...
        if line.contains("Opus") && (line.contains("transcode") || line.contains("Transcode")) {
            if !result
                .detected_defects
                .contains(&"OpusTranscode".to_string())
            {
                result.detected_defects.push("OpusTranscode".to_string());
            }
        }
        if (line.contains("Vorbis") || line.contains("Ogg"))
            && (line.contains("transcode") || line.contains("Transcode"))
        {
            if !result
                .detected_defects
                .contains(&"OggVorbisTranscode".to_string())
            {
                result
                    .detected_defects
                    .push("OggVorbisTranscode".to_string());
            }
        }
        if line.contains("Bit depth mismatch") || line.contains("BitDepthMismatch") {
            if !result
                .detected_defects
                .contains(&"BitDepthMismatch".to_string())
            {
...
fn compare_control_vs_transcoded() {
    ...
    for r in &control_results {
        let result_type = if r.detected_defects.is_empty() {
            "CLEAN"
        } else {
            &r.detected_defects.join(",")
        };
        println!(
            "{:<40} {:>10.0}Hz {:>10.1}% {:>12.1}dB/oct {:>15}",
        ...
        let mut builder = AllureTestBuilder::new(&format!("Compare Control: {}", r.filename))
            .full_name(&format!(
                "diagnostic_test::compare::control::{}",
                sanitize_name(&r.filename)
            ))
    ...
    if !control_results.is_empty() && !mp3_results.is_empty() {
        let ctrl_cutoff_avg = control_results.iter().map(|r| r.cutoff_ratio).sum::<f32>()
            / control_results.len() as f32;
        let mp3_cutoff_avg =
            mp3_results.iter().map(|r| r.cutoff_ratio).sum::<f32>() / mp3_results.len() as f32;

        let ctrl_steep_avg = control_results
            .iter()
            .map(|r| r.rolloff_steepness)
            .sum::<f32>()
            / control_results.len() as f32;
        let mp3_steep_avg =
            mp3_results.iter().map(|r| r.rolloff_steepness).sum::<f32>() / mp3_results.len() as f32;
        ...
        println!(
            ...
            (ctrl_cutoff_avg - mp3_cutoff_avg) * 100.0
        );
        ...
        println!(
            "  Cutoff ratio threshold: {:.1}% (midpoint)",
            suggested_cutoff * 100.0
        );
        println!(
            "  Steepness threshold: {:.1} dB/oct (midpoint)",
            suggested_steep
        );
        println!("\n  For safety margin, consider:");
        println!(
            "  - Cutoff ratio: < {:.1}% to flag as transcode",
            (suggested_cutoff - 0.05) * 100.0
        );
        println!(
            "  - Steepness: > {:.1} dB/oct required for MP3",
            suggested_steep + 5.0
        );
    }
