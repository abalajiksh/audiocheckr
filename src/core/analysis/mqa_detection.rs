...
//! - LSB noise patterns (MQA uses lower bits for encoding)
...
impl MqaDetector {
    ...
    pub fn detect(&self, samples: &[f32], sample_rate: u32, bit_depth: u32) -> MqaDetectionResult {
        ...
        // MQA commonly appears in 24-bit (hi-res container) and also in 16-bit (MQA-CD style) at 44.1/48kHz.
        if (bit_depth != 24 && bit_depth != 16) || (sample_rate != 44100 && sample_rate != 48000) {
            result
                .evidence
                .push("Not 16/24-bit 44.1/48kHz format (common MQA container formats)".to_string());
            return result;
        }
        ...
        result.lsb_entropy = self.analyze_lsb_entropy(samples, bit_depth);

        // 2. Analyze LSB correlation patterns (MQA has specific patterns)
        result.lsb_correlation = self.analyze_lsb_correlation(samples, bit_depth);
        ...
        result.bit_pattern_score = self.detect_bit_patterns(samples, bit_depth);

        // 6. Detect spectral folding artifacts
        result.spectral_folding_score = self.detect_spectral_folding(samples, sample_rate);

        // 7. Analyze LSB periodicity (early encoders have specific patterns)
        result.lsb_periodicity_score = self.analyze_lsb_periodicity(samples, bit_depth);

        // 8. Analyze bit transition rate
        result.bit_transition_rate = self.analyze_bit_transitions(samples, bit_depth);

        // 9. Analyze LSB value clustering
        result.lsb_value_clustering = self.analyze_lsb_clustering(samples, bit_depth);
        ...
        if self.detect_early_encoders {
            ...
            // Lowered threshold to make early encoder detection more accessible
        ...
        let has_correlation = result.lsb_correlation > 0.06;
        let has_hf_noise = result.hf_noise_level > -75.0;

        // If we have entropy + one other signal, we likely have MQA
        let basic_mqa_indicators = if has_elevated_entropy {
            let mut count = 0;
            if has_noise_elevation {
                count += 1;
            }
            if has_correlation {
                count += 1;
        ...
        } else if result.lsb_entropy > 0.35 {
        ...
        // LSB correlation patterns
        if result.lsb_correlation > 0.06 {
        ...
        if result.spectral_folding_score > 0.20 {
        ...
        if result.lsb_periodicity_score > 0.15 {
        ...
        if result.lsb_value_clustering > 0.25 {
        ...
        if result.is_mqa_encoded {
            ...
            if let Some(ref version) = result.encoder_version {
                result
                    .evidence
                    .push(format!("Detected MQA encoder version: {:?}", version));
            }
        }
    ...
    /// Convert normalized float sample to an integer range appropriate for the container bit depth.
    #[inline]
    fn sample_to_container_int(sample: f32, bit_depth: u32) -> i32 {
        let sample_clamped = sample.clamp(-1.0, 1.0);

        // NOTE: This detector is tuned for 16-bit and 24-bit containers.
        let max = match bit_depth {
            16 => 32767.0,
            24 => 8388607.0,
            _ => {
                // Best-effort fallback for unusual depths.
                // Avoid shifting by >= 31.
                let bd = bit_depth.clamp(2, 30);
                ((1u32 << (bd - 1)) - 1) as f32
            }
        };

        (sample_clamped * max) as i32
    }
    ...
    /// Analyze entropy in the least significant byte.
    fn analyze_lsb_entropy(&self, samples: &[f32], bit_depth: u32) -> f32 {
        let mut lsb_values = Vec::with_capacity(samples.len());

        for &sample in samples {
            let int_val = Self::sample_to_container_int(sample, bit_depth);
            let lsb = (int_val.abs() & 0xFF) as u8;
            lsb_values.push(lsb);
        }

        // Degenerate case (all zeros or single value)
    ...
    fn analyze_lsb_correlation(&self, samples: &[f32], bit_depth: u32) -> f32 {
        if samples.len() < 1000 {
            return 0.0;
        }
        ...
        for &sample in samples {
            let int_val = Self::sample_to_container_int(sample, bit_depth);
            let lsb = (int_val.abs() & 0xFF) as i32;
            lsb_values.push(lsb);
        }
        ...
        let variance: f32 = lsb_values
            .iter()
            .map(|&x| (x as f32 - mean).powi(2))
            .sum::<f32>()
            / lsb_values.len() as f32;
    ...
    fn analyze_lsb_periodicity(&self, samples: &[f32], bit_depth: u32) -> f32 {
        if samples.len() < 4096 {
            return 0.0;
        }
    ...
    fn analyze_bit_transitions(&self, samples: &[f32], bit_depth: u32) -> f32 {
        if samples.len() < 1000 {
            return 0.5;
        }
    ...
    fn analyze_lsb_clustering(&self, samples: &[f32], bit_depth: u32) -> f32 {
        if samples.len() < 1000 {
            return 0.0;
        }
    ...
    fn detect_bit_patterns(&self, samples: &[f32], bit_depth: u32) -> f32 {
        if samples.len() < 4096 {
            return 0.0;
        }
        ...
        for &sample in samples.iter().take(16384) {
            let int_val = Self::sample_to_container_int(sample, bit_depth);
            let lsb = (int_val.abs() & 0xFF) as u8;
            byte_stream.push(lsb);
        }

        let mut pattern_score = 0.0f32;

        // Check for non-random distribution of byte transitions
        let mut transitions = 0;
        for i in 1..byte_stream.len() {
            if byte_stream[i] != byte_stream[i - 1] {
                transitions += 1;
            }
        }

        let transition_rate = transitions as f32 / (byte_stream.len() - 1) as f32;
        ...
        let period_scores = self.check_bit_periodicity(&byte_stream);
    ...
    fn detect_spectral_folding(&self, samples: &[f32], sample_rate: u32) -> f32 {
        ...
        let fold_points = [sample_rate as f32 / 4.0, sample_rate as f32 / 3.0];
...
mod tests {
    ...
    fn test_lsb_entropy() {
        ...
        let entropy_24 = detector.analyze_lsb_entropy(&samples, 24);
        assert!(entropy_24 > 0.0 && entropy_24 <= 1.0);

        let entropy_16 = detector.analyze_lsb_entropy(&samples, 16);
        assert!(entropy_16 >= 0.0 && entropy_16 <= 1.0);
    }
    ...
    fn test_lsb_periodicity() {
        ...
        let periodicity = detector.analyze_lsb_periodicity(&samples, 24);
    ...
    fn test_lsb_clustering() {
        ...
        let clustering = detector.analyze_lsb_clustering(&samples, 24);
    ...
    #[test]
    fn test_accepts_16_bit_container() {
        let detector = MqaDetector::default();
